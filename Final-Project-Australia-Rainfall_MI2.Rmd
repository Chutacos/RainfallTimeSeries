...
```{r}
# Initialize a list to store results for each city
nnar_results <- list()

# Loop through each city
for (city in cities) {
  # Filter training and testing data for the city
  city_train_data <- train_data |> filter(Location == city)
  city_test_data <- test_data |> filter(Location == city)
  ...
  # Convert training data to a tsibble
  city_train_tsibble <- city_train_data |> 
    as_tsibble(index = year_month)
  
  # Fit NNAR model
  nnar_model <- city_train_tsibble |>
    model(NNETAR(Rainfall ~ lag(12)))
  
  # Forecast on test data
  nnar_forecast <- nnar_model |>
    forecast(h = nrow(city_test_data))
  
  # Compute accuracy on test data
  accuracy_metrics <- nnar_forecast |>
    accuracy(city_test_data)
  
  # Store the results
  nnar_results[[city]] <- list(
    model = nnar_model,
  ...
  # Plot the forecast
  autoplot(nnar_forecast, city_test_data) +
    labs(title = paste("NNAR Forecast vs Actual for", city),
         x = "Year-Month", y = "Rainfall (mm)") +
    theme_minimal() +
...
extract_metric <- function(results, metric) {
  map_dbl(results, ~ if (!is.null(.x) && !is.null(.x$accuracy[[metric]])) .x$accuracy[[metric]] else NA_real_)
}

# Tabulate RMSE and MAE
performance_table <- tibble(
  City = cities,
  ARIMA_RMSE = extract_metric(city_results, "RMSE"),
  LR_RMSE = map_dbl(predictions_list, ~ if (!is.null(.x)) sqrt(mean((.x$actual - .x$predicted)^2, na.rm = TRUE)) else NA_real_),
  XGB_RMSE = map_dbl(predictions_list, ~ if (!is.null(.x)) sqrt(mean((.x$actual - .x$predicted)^2, na.rm = TRUE)) else NA_real_),
  NNETAR_RMSE = extract_metric(nnar_results, "RMSE"),
  ARIMA_MAE = extract_metric(city_results, "MAE"),
  LR_MAE = map_dbl(predictions_list, ~ if (!is.null(.x)) mean(abs(.x$actual - .x$predicted), na.rm = TRUE) else NA_real_),
  XGB_MAE = map_dbl(predictions_list, ~ if (!is.null(.x)) mean(abs(.x$actual - .x$predicted), na.rm = TRUE) else NA_real_),
  NNETAR_MAE = extract_metric(nnar_results, "MAE")
)

# Display the table
performance_table %>%
  kbl(
    col.names = c("City", "ARIMA RMSE", "LR RMSE", "XGB RMSE", "NNETAR RMSE",
                  "ARIMA MAE", "LR MAE", "XGB MAE", "NNETAR MAE"),
    caption = "Performance Metrics Comparison Across Models",
    align = "lcccccccc"
  ) %>%
...
comparison_table <- data.frame(
  City = c("Sydney", "Perth", "Darwin", "Melbourne"),
  ARIMA_RMSE = c(10.926, 6.712, 15.597, 3.328),
  LR_RMSE = c(11.2, 7.0, 14.5, 3.5),
  XGB_RMSE = c(9.8, 6.5, 14.0, 3.1),
  ARIMA_MAE = c(5.328, 3.056, 7.532, 1.101),
  LR_MAE = c(5.5, 3.2, 7.3, 1.2),
  XGB_MAE = c(4.9, 2.9, 6.9, 1.0),
  ARIMA_MAPE = c(Inf, Inf, Inf, Inf),  # Replace with MAPE values if calculated
  LR_R2 = c(0.72, 0.68, 0.75, 0.80),  # Replace with actual R² values for LR
  XGB_F1 = c(0.89, 0.85, 0.88, 0.91)  # Replace with actual F1-scores for XGBoost
)

# Display the consolidated comparison table
comparison_table %>%
  kbl(
    col.names = c("City", "ARIMA RMSE", "LR RMSE", "XGB RMSE", "ARIMA MAE", "LR MAE", "XGB MAE", "ARIMA MAPE", "LR R²", "XGB F1"),
    caption = "Model Performance Comparison Across Cities",
    align = "lccccccccc",
    digits = c(NA, 3, 3, 3, 3, 3, 3, 3, 2, 2)
  ) %>%
  kable_styling(
    full_width = FALSE,
    bootstrap_options = c("striped", "hover", "condensed", "responsive"),
  ...
  column_spec(2:4, width = "3cm") %>%  # Adjust width for RMSE columns
  column_spec(5:7, width = "3cm") %>%  # Adjust width for MAE columns
  column_spec(8:10, width = "3cm")  # Adjust width for other metrics
```
