...
# Add XGBoost results dynamically
for (city in cities) {
  if (city %in% names(predictions_list)) {
    xgboost_predictions <- predictions_list[[city]]$predicted
    xgboost_actuals <- predictions_list[[city]]$actual
    if (!is.null(xgboost_predictions) && !is.null(xgboost_actuals)) {
      rmse_value <- sqrt(mean((xgboost_actuals - xgboost_predictions)^2, na.rm = TRUE))
      mae_value <- mean(abs(xgboost_actuals - xgboost_predictions), na.rm = TRUE)
      add_metrics("XGBoost", city, rmse_value, mae_value)
    }
  }
}
